...
function mystic_market($today, $mon_to_thurs, $weekends, $current_time, $open_img, $close_img){
  ...
  $main_open_soon = date("H:i A", strtotime("$open-30min"));
  $late_open_soon = date("H:i A", strtotime("$late_open-30min"));
  $main_close_soon = date("H:i A", strtotime("$close-30min"));
  $early_close_soon = date("H:i A", strtotime("$early_close-30min"));

  $open_display = date("g:i A", strtotime($open));
  $late_open_display = date("g:i A", strtotime($late_open));
  $close_display = date("g:i A", strtotime($close));
  ...
  if (in_array($today, $mon_to_thurs) && ($current_time >= $opening) && ($current_time < $closing)){
    ...
    echo $open_img;
  } elseif (in_array($today, $weekends) && ($current_time >= $late_opening) && ($current_time < $early_closing)){
    echo $open_img;
  } else {
    echo $close_img;
  }
  ...
  if (((in_array($today, $mon_to_thurs) || ($today == "Friday")) && ($current_time >= $main_open_soon) && ($current_time < $opening))){
    echo "<h2 class='notify-opening'> Opening at " . $open_display;
  } elseif ((in_array($today, $weekends) && ($current_time >= $late_open_soon) && ($current_time < $late_opening))){
    echo "<h2 class='notify-opening'> Opening at " . $late_open_display;
  } elseif ((in_array($today, $mon_to_thurs) && ($current_time >= $main_close_soon) && ($current_time < $closing))){
    echo "<h2 class='notify-closing'> Closing at " . $close_display;
  } elseif (((in_array($today, $weekends) || ($today == "Friday")) && ($current_time >= $early_close_soon) && ($current_time < $early_closing))){
    echo "<h2 class='notify-closing'> Closing at ". $early_close_display .'</h2>';
  }
}
...
function village_greens($today, $weekdays, $mon_to_thurs, $current_time, $open_img, $close_img){
  ...
  $late_open_soon = date("H:i A", strtotime("$late_open-30min"));
  $close_soon = date("H:i A", strtotime("$close-30min"));
  $late_close_soon = date("H:i A", strtotime("$late_close-30min"));
  ...
  if (in_array($today, $weekdays) && (($current_time >= $open_soon) && ($current_time < $opening))){
    echo "<h2 class='notify-opening'> Opening at " . $open_display . "</h2>";
  } elseif (in_array($today, $weekdays) && (($current_time >= $close_soon) && ($current_time < $closing))){
    echo "<h2 class='notify-closing'> Closing at " . $close_display . "</h2>";
  } elseif ((in_array($today, $mon_to_thurs) || ($today == "Sunday")) && (($current_time >= $late_open_soon) && ($current_time < $late_opening))){
    echo "<h2 class='notify-opening'> Opening at " .$late_open_display . "</h2>";
  } elseif ((in_array($today, $mon_to_thurs) || ($today == "Sunday")) && (($current_time >= $late_close_soon) && ($current_time < $late_closing))){
      echo "<h2 class='notify-closing'> Closing at " . $late_close_display . "</h2>";
  }
}
